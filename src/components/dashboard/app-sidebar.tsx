...
const formSchema = z.object({
  ...
  salaryRange: z.tuple([z.number().min(0), z.number().min(0)]).refine(([min, max]) => min <= max, {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Sidebar>
          <SidebarHeader
            className={"flex flex-row items-center justify-between gap-4 px-4 py-4 md:pt-16"}
          >
                    ...
                    >
                      {genders.map(({ id, gender }) => (
                        <FormItem key={id} className="flex items-center gap-2">
                          <FormControl>
                            <RadioGroupItem value={id.toString()} id={`gender-${id}`} />
                          </FormControl>
